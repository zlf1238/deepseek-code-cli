...
test("buildSlashCommands prefixes skills before built-ins", () => {
  ...
  assert.deepEqual(builtinNames, ["new", "resume", "skills", "model", "thinking", "autoSwitch", "verbose", "exit", "learn", "worklog", "compact"]);
});
...
test("filterSlashCommands returns only built-in commands on bare slash", () => {
  const items = buildSlashCommands(skills);
  const matched = filterSlashCommands(items, "/");
  const matchedNames = matched.map((i) => i.name);
  assert.deepEqual(matchedNames, ["new", "resume", "skills", "model", "thinking", "autoSwitch", "verbose", "exit", "learn", "worklog", "compact"]);
  assert.ok(matched.every((i) => i.kind !== "skill"));
});

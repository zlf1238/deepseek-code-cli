...
import { extractSegments, normalizeTerminalOutput, sliceByColumn, sliceWithWidth, truncateToWidth, visibleWidth } from "./utils";
	...
	private doRender(): void {
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				// Auto-truncate over-wide lines instead of crashing.
				// This guards against edge cases where component width calculation
				// disagrees with visibleWidth (e.g. CJK table rendering).
				if (process.env.PI_TUI_DEBUG === "1") {
					const crashLogPath = path.join(os.homedir(), ".pi", "agent", "pi-truncation.log");
					const crashData = [
						`Truncation at ${new Date().toISOString()}`,
...
// 桩函数 - 替换 pi 的 keys.ts 和 terminal-image.ts 依赖
function isKeyRelease(_data: string): boolean { return false; }
function matchesKey(_data: string, _key: unknown): boolean { return false; }
function isImageLine(_line: string): boolean { return false; }
function deleteKittyImage(_id: number): string { return ""; }
function deleteAllKittyImages(_ids: Iterable<number>): string { return ""; }
function getCapabilities() { return { kitty: false, iterm2: false, sixel: false, images: false }; }
function setCellDimensions(_dims: { widthPx: number; heightPx: number }): void {}

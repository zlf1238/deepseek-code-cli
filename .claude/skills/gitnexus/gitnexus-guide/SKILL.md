...
2. **将你的任务与下面的技能匹配**并**读取对应的 skill 文件**
3. **遵循对应技能的工作流和检查清单**

> 如果步骤 1 警告索引过期，先在终端中运行 `npx gitnexus analyze`。

## 技能

| 任务                                           | 需读取的技能                |
| ---------------------------------------------- | --------------------------- |
| 了解架构 / "X 是如何工作的？"                   | `gitnexus-exploring`        |
| 影响范围 / "修改 X 会破坏什么？"                | `gitnexus-impact-analysis`  |
| 追踪 Bug / "X 为什么失败？"                     | `gitnexus-debugging`        |
| 重命名 / 提取 / 拆分 / 重构                     | `gitnexus-refactoring`      |
| 工具、资源、Schema 参考                         | `gitnexus-guide`（当前文件） |
...
| `impact`           | 符号影响范围——深度 1/2/3 的破坏范围及置信度                         |
...
| 资源                                             | 内容                             |
| ------------------------------------------------ | -------------------------------- |
| `gitnexus://repo/{name}/context`                 | 统计数据、过期检查                |
| `gitnexus://repo/{name}/clusters`                | 所有功能领域及内聚度评分           |
| `gitnexus://repo/{name}/cluster/{clusterName}`   | 领域成员                         |
